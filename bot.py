...
def privileged(update: telegram.Update, command: str, check_only: bool = True) -> bool:
    ...
    if update.effective_chat.id in admin_chat_ids:
        if not check_only:
            logging.warning(
                f"Command {command} by ID {update.effective_chat.id}: {update.message.text}"
            )
        return True
    else:
        if not check_only:
            logging.warning(
                f"Unauthorized {command} attempted by ID {update.effective_chat.id}."
            )
        return False


def parse_argument(text: str, key: str) -> dict:
    arg = re.search(r"{key}=(.*?)(?:\s|$)", text)
...
async def new_sub(
    update: telegram.Update, context: ContextTypes.DEFAULT_TYPE, reenable: bool = False
) -> None:
    ...
    if reenable:
        hestia.query_db(
            "UPDATE hestia.subscribers SET telegram_enabled = true WHERE telegram_id = %s",
            params=[str(update.effective_chat.id)],
        )
    else:
        hestia.query_db(
            "INSERT INTO hestia.subscribers VALUES (DEFAULT, '2099-01-01T00:00:00', DEFAULT, DEFAULT, DEFAULT, DEFAULT, true, %s)",
            params=[str(update.effective_chat.id)],
        )
...
async def start(update: telegram.Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    checksub = hestia.query_db(
        "SELECT * FROM hestia.subscribers WHERE telegram_id = %s",
        params=[str(update.effective_chat.id)],
        fetchOne=True,
    )
...
async def stop(update: telegram.Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if checksub is not None:
        if checksub["telegram_enabled"]:
            ...
            hestia.query_db(
                "UPDATE hestia.subscribers SET telegram_enabled = false WHERE telegram_id = %s",
                params=[str(update.effective_chat.id)],
            )
    ...
    donation_link = hestia.query_db(
        "SELECT donation_link FROM hestia.meta", fetchOne=True
    )["donation_link"]
...
async def announce(update: telegram.Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not privileged(update, "announce", check_only=False):
        return

    if hestia.check_dev_mode():
        subs = hestia.query_db(
            "SELECT * FROM hestia.subscribers WHERE subscription_expiry IS NOT NULL AND telegram_enabled = true AND user_level > 1"
        )
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text="Dev mode is enabled, message not broadcasted to all subscribers.",
        )
    else:
        subs = hestia.query_db(
            "SELECT * FROM hestia.subscribers WHERE subscription_expiry IS NOT NULL AND telegram_enabled = true"
        )
        ...
        try:
            if markdown["value"]:
                await context.bot.send_message(
                    ...
                    msg,
                    parse_mode="MarkdownV2",
                    disable_web_page_preview=bool(disablepreview["value"]),
                )
            else:
                await context.bot.send_message(
                    sub["telegram_id"],
            ...
            logging.warning(
                f"Exception while broadcasting announcement to {sub['telegram_id']}: {repr(e)}"
            )
            continue


async def websites(update: telegram.Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    targets = hestia.query_db(
        "SELECT agency, user_info FROM hestia.targets WHERE enabled = true"
    )
...
async def get_sub_info(
    ...
    if not privileged(update, "get_sub_info", check_only=False):
        return
...
async def halt(update: telegram.Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not privileged(update, "halt", check_only=False):
        return

    hestia.query_db(
        "UPDATE hestia.meta SET scraper_halted = true WHERE id = %s",
        params=[hestia.SETTINGS_ID],
    )
...
async def resume(update: telegram.Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not privileged(update, "resume", check_only=False):
        return

    settings = hestia.query_db(
        "SELECT scraper_halted FROM hestia.meta WHERE id = %s",
        params=[hestia.SETTINGS_ID],
        fetchOne=True,
    )
    ...
    if settings["scraper_halted"]:
        hestia.query_db(
            "UPDATE hestia.meta SET scraper_halted = false WHERE id = %s",
            params=[hestia.SETTINGS_ID],
        )
...
async def enable_dev(
    update: telegram.Update, context: ContextTypes.DEFAULT_TYPE
) -> None:
    if not privileged(update, "dev", check_only=False):
        return
...
async def disable_dev(
    update: telegram.Update, context: ContextTypes.DEFAULT_TYPE
) -> None:
    if not privileged(update, "nodev", check_only=False):
        return

    hestia.query_db(
        "UPDATE hestia.meta SET devmode_enabled = false WHERE id = %s",
        params=[hestia.SETTINGS_ID],
    )
...
async def get_all_subs(
    ...
    if not privileged(update, "get_all_subs", check_only=False):
        return
...
async def status(update: telegram.Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not privileged(update, "status", check_only=False):
        return

    settings = hestia.query_db(
        "SELECT * FROM hestia.meta WHERE id = %s",
        params=[hestia.SETTINGS_ID],
        fetchOne=True,
    )
    ...
    active_sub_count = hestia.query_db(
        "SELECT COUNT(*) FROM hestia.subscribers WHERE telegram_enabled = true",
        fetchOne=True,
    )
    sub_count = hestia.query_db(
        "SELECT COUNT(*) FROM hestia.subscribers", fetchOne=True
    )
    message += "\n"
    ...
    donation_link = hestia.query_db(
        "SELECT donation_link, donation_link_updated FROM hestia.meta", fetchOne=True
    )
    message += "\n"
    ...
    for target in targets:
        ...
        target_id = target["id"]
        count = hestia.query_db(
            "SELECT COUNT(*) FROM hestia.homes WHERE agency = %s AND date_added > now() - '1 week'::interval",
            params=[agency],
            fetchOne=True,
        )
    ...
    await context.bot.send_message(
        update.effective_chat.id, message, disable_web_page_preview=True
    )


async def set_donation_link(
    update: telegram.Update, context: ContextTypes.DEFAULT_TYPE
) -> None:
    if not privileged(update, "setdonationlink", check_only=False):
        return

    link = update.message.text.split(" ")[1]

    hestia.query_db(
        "UPDATE hestia.meta SET donation_link = %s, donation_link_updated = %s WHERE id = %s",
        params=[link, datetime.now().isoformat(), hestia.SETTINGS_ID],
    )
...
async def filter(update: telegram.Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Set minprice filter
        ...
        hestia.query_db(
            "UPDATE hestia.subscribers SET filter_min_price = %s WHERE telegram_id = %s",
            params=(minprice, str(update.effective_chat.id)),
        )
    ...
    # Set maxprice filter
        ...
        hestia.query_db(
            "UPDATE hestia.subscribers SET filter_max_price = %s WHERE telegram_id = %s",
            params=(maxprice, str(update.effective_chat.id)),
        )
    ...
    # View city possibilities
        ...
        all_filter_cities = [
            c["city"] for c in hestia.query_db("SELECT DISTINCT city FROM hestia.homes")
        ]
    ...
    # Modify agency filter
        ...
        enabled_agencies = hestia.query_db(
            "SELECT filter_agencies FROM subscribers WHERE telegram_id = %s",
            params=[str(update.effective_chat.id)],
            fetchOne=True,
        )["filter_agencies"]
        for row in hestia.query_db(
            "SELECT agency, user_info FROM hestia.targets WHERE enabled = true"
        ):
            if row["agency"] not in included:
                ...
                if row["agency"] in enabled_agencies:
                    reply_keyboard.append(
                        [
                            telegram.InlineKeyboardButton(
                                hestia.CHECK_EMOJI + " " + row["user_info"]["agency"],
                                callback_data=f"hfa.d.{row['agency']}",
                            )
                        ]
                    )
                else:
                    reply_keyboard.append(
                        [
                            telegram.InlineKeyboardButton(
                                hestia.CROSS_EMOJI + " " + row["user_info"]["agency"],
                                callback_data=f"hfa.e.{row['agency']}",
                            )
                        ]
                    )

        await update.message.reply_text(
            "Select the agencies you want to receive updates from",
    ...
    # Modify city filter
        ...
        len(cmd) >= 4
        and cmd[1] == "city"
        and cmd[2] in ["add", "remove", "rm", "delete", "del"]
    ):
        city = ""
        for token in cmd[3:]:
        ...
        sub_filter_cities = hestia.query_db(
            "SELECT filter_cities FROM hestia.subscribers WHERE telegram_id = %s",
            params=[str(update.effective_chat.id)],
            fetchOne=True,
        )["filter_cities"]
        ...
        if cmd[2] == "add":
            ...
            all_filter_cities = [
                c["city"]
                for c in hestia.query_db("SELECT DISTINCT city FROM hestia.homes")
            ]
            all_filter_cities.sort()
            ...
            hestia.query_db(
                "UPDATE hestia.subscribers SET filter_cities = %s WHERE telegram_id = %s",
                params=(
                    str(sub_filter_cities).replace("'", '"'),
...
async def callback_query_handler(update: telegram.Update, _) -> None:
    ...
    if cbid == "hfa":
        ...
        enabled_agencies = set(
            hestia.query_db(
                "SELECT filter_agencies FROM hestia.subscribers WHERE telegram_id = %s",
                params=[str(query.message.chat.id)],
                fetchOne=True,
            )["filter_agencies"]
        )
        if action == "d":
        ...
        hestia.query_db(
            "UPDATE hestia.subscribers SET filter_agencies = %s WHERE telegram_id = %s",
            params=(
                str(list(enabled_agencies)).replace("'", '"'),
        ...
        await query.edit_message_reply_markup(
            telegram.InlineKeyboardMarkup(reply_keyboard)
        )
...
if __name__ == "__main__":
    initialize()
    application = ApplicationBuilder().token(secrets_1.TOKEN).build()
